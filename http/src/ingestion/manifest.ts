...
export const loadManifest = (cacheDir: string): IndexManifest => {
  ...
  try {
    ...
    if (parsed.version !== 1) {
      console.error(
        `[ts-graph] Unknown manifest version ${parsed.version}, starting fresh`,
      );
      return { version: 1, files: {} };
    }
    ...
    console.error(
      `[ts-graph] Failed to load manifest: ${error instanceof Error ? error.message : String(error)}`,
    );
    return { version: 1, files: {} };
  }
};
...
export const saveManifest = (
  ...
  try {
    ...
    console.error(
      `[ts-graph] Failed to save manifest: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
};
