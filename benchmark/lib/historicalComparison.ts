 ...
 */
export function printHistoricalComparison(comparisons: HistoricalComparison[]): void {
	// Filter to only comparisons with baseline data
	const withBaseline = comparisons.filter((c) => c.historicalWithoutMcp !== null);
	...
	for (const c of withBaseline) {
		...
		const time = formatMetricChange(current.avgDurationMs, baseline.avgDurationMs, "time");
		const cost = formatMetricChange(current.avgCostUsd, baseline.avgCostUsd, "cost");
		...
		if (c.historicalWithMcp && c.historicalWithMcp.runCount > 0) {
			...
			const trendTime = formatMetricChange(current.avgDurationMs, past.avgDurationMs, "time");
			const trendCost = formatMetricChange(current.avgCostUsd, past.avgCostUsd, "cost");

			console.log(`  vs ${past.runCount} past WITH_MCP runs:`);
			console.log(`    Time: ${trendTime.icon} ${trendTime.pct} ${trendTime.direction}`);
			console.log(`    Cost: ${trendCost.icon} ${trendCost.pct} ${trendCost.direction}`);
		}
	}
}
