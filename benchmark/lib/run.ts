...
export async function runBenchmarks(
  ...
  if (skippedTasks.length > 0) {
    console.log(
      `Tasks:        ${totalTasks} to run (${totalPossible - totalTasks} skipped from history)`,
    );
  } else {
  ...
  if (skippedTasks.length > 0) {
    ...
    for (const skip of skippedTasks) {
      console.log(
        `  ${skip.prompt.id} | ${skip.scenario.id}: ${skip.count} runs in history`,
      );
    }
  }
  console.log("");
  ...
  // Run tasks
  console.log(
    `Running ${totalTasks} tasks (${options.concurrency} parallel)...\n`,
  );
  ...
  const updatedHistory = await appendRuns(
    config.projectName,
    runs,
    selectedPrompts,
  );
...
async function main() {
  ...
  if (!projectPath) {
    console.error(
      "Usage: npx tsx benchmark/lib/run.ts <test-project-path> [options]",
    );
    console.error("");
    console.error("Options:");
    console.error(
      "  --runs, -r <n>       Number of iterations per prompt/scenario (default: 1)",
    );
    console.error("  --prompt <id>        Run specific prompt only (e.g., P1)");
    console.error(
      "  --scenario <id>      Run specific scenario only (e.g., with-mcp)",
    );
    console.error(
      "  --concurrency, -c <n> Number of parallel tasks (default: 3)",
    );
    ...
    console.error(
      "  --min-runs <n>       Min WITHOUT_MCP runs before skipping (default: 5)",
    );
    console.error("");
