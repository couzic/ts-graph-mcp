 ...
 */
export async function loadHistory(projectName: string): Promise<BenchmarkHistory> {
	...
	try {
		...
		if (history.version !== 1) {
			console.warn(`History version mismatch: expected 1, got ${history.version}. Starting fresh.`);
			return createEmptyHistory(projectName);
		}
...
export async function appendRuns(
	...
	for (const run of runs) {
		...
		if (!promptDef) {
			console.warn(`Prompt ${run.promptId} not found in prompts array, skipping run`);
			continue;
		}
		...
		const existingIndex = runsArray.findIndex((r) => r.timestamp === run.timestamp);
		if (existingIndex === -1) {
			runsArray.push(run);
		}
	}
